...
+(NSDictionary*) all
{
    ...
    if (dict == nil) {
        ...
        @"function2D"  : @"x^6-2*x^4+x^2", // x^6-2*x^4+x^2, x^5−2*x^3+x
        @"function3D"  : @"(x^2)/y",

        @"arithmeticFunctions" : @YES,
        @"logarithmicFunctions" : @NO,
        @"goniometricFunctions" : @YES,
        @"polynomialFunctions" : @YES,
        
        @"otherFunctions" : @YES,
        
        @"exponentialFunction" : @NO,
        ...
        @"forcebit6": @YES, //cos
        @"forcebit7": @NO, //tan
        @"forcebit8": @YES, //log   TODO: bug when @NO?
        @"forcebit9": @NO, //log2
        @"forcebit10": @NO, //log10
        @"forcebit11": @YES, //^2
        @"forcebit12": @YES, //^3
...
+(NSMutableArray*) ekvidistantDoublesCount:(int)count min:(double)m max:(double)mx {
    ...
    for (int i=0; i<count; i++) {
        if (qx+rx-dx>-0.005 &&qx+rx-dx<0.005 ) {
            [array insertObject:@0.0 atIndex:i ];
        }
        else [array insertObject:@(qx+rx+dx) atIndex:i ];
        qx=qx+rx;
    }
    return array;
...
-(instancetype)initWith:(double *)n name:(NSString *)s{
    self = [super init];
    if (self) {
        variable=n;
        name=s;
...
-(instancetype)initWith:(double *)n {
    self = [super init];
    if (self) {
        constant=n;
...
- (instancetype)initWith:(func2) f name:(NSString *)n{
    self = [super init];
    if (self) {
        function=f;
...
- (instancetype)initWith:(func1) f name:(NSString *)n{
    self = [super init];
    if (self) {
        function=f;
        name=[[NSString alloc]initWithString:n];
...
- (instancetype)initWith:(func0) f name:(NSString *)n{
    self = [super init];
    if (self) {
        function=f;
        name=[[NSString alloc]initWithString:n];
...
- (instancetype)initWithArray:(int [])n andGFS:(GFS *)g repairing:(int [])rep{
    self = [super init];
    if (self) {
        gfs=g;
        array=n;
...
-(instancetype)initWithString:(NSString *)s andGFS:(GFS*)g {
    self = [super init];
    if (self) {
        ...
        gfsbinstr=[NSString stringWithString:lines[1]];
        astr=[NSString stringWithString:lines[2]];
        bstr=[NSString stringWithString:lines[3]];
        name=[NSString stringWithString:lines[4]];
        tex=[NSString stringWithString:lines[5]];
...
-(NSDictionary*) mask:(uint64) b
{
    NSDictionary* dict = @{
      ...
      // arithmeticFunctions:
      @"add" : (((b&bit1)==bit1)||([[Configuration all][@"forcebit1"]isEqualTo:@YES]))?@YES:@NO,
      @"sub" : (((b&bit2)==bit2)||([[Configuration all][@"forcebit2"]isEqualTo:@YES]))?@YES:@NO,
      @"mul" : (((b&bit3)==bit3)||([[Configuration all][@"forcebit3"]isEqualTo:@YES]))?@YES:@NO,
      @"div" : (((b&bit4)==bit4)||([[Configuration all][@"forcebit4"]isEqualTo:@YES]))?@YES:@NO,
      
      // goniometricFunctions:
      @"sin" : (((b&bit5)==bit5)||([[Configuration all][@"forcebit5"]isEqualTo:@YES]))?@YES:@NO,
      @"cos" : (((b&bit6)==bit6)||([[Configuration all][@"forcebit6"]isEqualTo:@YES]))?@YES:@NO,
      @"tan" : (((b&bit7)==bit7)||([[Configuration all][@"forcebit7"]isEqualTo:@YES]))?@YES:@NO,
      
      // logarithmicFunctions:
      @"log" : (((b&bit8)==bit8)||([[Configuration all][@"forcebit8"]isEqualTo:@YES]))?@YES:@NO,
      @"log2" : (((b&bit9)==bit9)||([[Configuration all][@"forcebit9"]isEqualTo:@YES]))?@YES:@NO,
      @"log10" : (((b&bit10)==bit10)||([[Configuration all][@"forcebit10"]isEqualTo:@YES]))?@YES:@NO,
      
      // polynomialFunctions:
      @"pow2" : (((b&bit11)==bit11)||([[Configuration all][@"forcebit11"]isEqualTo:@YES]))?@YES:@NO,
      @"pow3" : (((b&bit12)==bit12)||([[Configuration all][@"forcebit12"]isEqualTo:@YES]))?@YES:@NO,
      @"pow4" : (((b&bit13)==bit13)||([[Configuration all][@"forcebit13"]isEqualTo:@YES]))?@YES:@NO,
      @"pow5" : (((b&bit14)==bit14)||([[Configuration all][@"forcebit14"]isEqualTo:@YES]))?@YES:@NO,
      @"pow6" : (((b&bit15)==bit15)||([[Configuration all][@"forcebit15"]isEqualTo:@YES]))?@YES:@NO, // Here is place for x,y according to 32th flag
      
      // linearConstants:
      @"1" : (((b&bit16)==bit16)||([[Configuration all][@"forcebit16"]isEqualTo:@YES]))?@YES:@NO,
      @"2" : (((b&bit17)==bit17)||([[Configuration all][@"forcebit17"]isEqualTo:@YES]))?@YES:@NO,
      @"3" : (((b&bit18)==bit18)||([[Configuration all][@"forcebit18"]isEqualTo:@YES]))?@YES:@NO,
      @"4" : (((b&bit19)==bit19)||([[Configuration all][@"forcebit19"]isEqualTo:@YES]))?@YES:@NO,
      @"5" : (((b&bit20)==bit20)||([[Configuration all][@"forcebit20"]isEqualTo:@YES]))?@YES:@NO,
      @"6" : (((b&bit21)==bit21)||([[Configuration all][@"forcebit21"]isEqualTo:@YES]))?@YES:@NO,
      @"7" : (((b&bit22)==bit22)||([[Configuration all][@"forcebit22"]isEqualTo:@YES]))?@YES:@NO,
      @"8" : (((b&bit23)==bit23)||([[Configuration all][@"forcebit23"]isEqualTo:@YES]))?@YES:@NO,
      @"9" : (((b&bit24)==bit24)||([[Configuration all][@"forcebit24"]isEqualTo:@YES]))?@YES:@NO,
      
      // exponentialFunction:
      @"^" : (((b&bit25)==bit25)||([[Configuration all][@"forcebit25"]isEqualTo:@YES]))?@YES:@NO,
      
      // otherFunctions:
      @"sqrt" : (((b&bit26)==bit26)||([[Configuration all][@"forcebit26"]isEqualTo:@YES]))?@YES:@NO,
      @"abs" : (((b&bit27)==bit27)||([[Configuration all][@"forcebit27"]isEqualTo:@YES]))?@YES:@NO,
      @"exp" : (((b&bit28)==bit28)||([[Configuration all][@"forcebit28"]isEqualTo:@YES]))?@YES:@NO,
      
      // mathConstants:
      @"pi" : (((b&bit29)==bit29)||([[Configuration all][@"forcebit29"]isEqualTo:@YES]))?@YES:@NO,
      @"e" : (((b&bit30)==bit30)||([[Configuration all][@"forcebit30"]isEqualTo:@YES]))?@YES:@NO,
      @"phi" : (((b&bit31)==bit31)||([[Configuration all][@"forcebit31"]isEqualTo:@YES]))?@YES:@NO,
      
      // 3D?
      @"y" : (((b&bit32)==bit32)||([[Configuration all][@"forcebit32"]isEqualTo:@YES]))?@YES:@NO,
      } ;
...
/*! Instantiate GFS with elements of pointers to functions wrapped in custom GFSelement subclasses according to number of parameters used
 \return self
 */
-(instancetype)initWithFunctionSet:(uint64)fsb andSeed:(uint32)seed{
    ...
    if (self) {
        ...
        elements=[[NSMutableArray alloc] initWithCapacity:32+4*MAXCONSTANTS]; // + possibly reinforced?
        ...
        if (([[Configuration all][@"arithmeticFunctions"]isEqualTo:@YES])||
            ([[Configuration all][@"linearConstants"]isEqualTo:@YES])){
        
            if (([fs[@"add"] isEqualTo:@YES])&&((![[Configuration all][@"forcebit1"]isEqualTo:@NO])||([[Configuration all][@"forcebit1"]isEqualTo:[NSNull null]]))) {
                [elements insertObject:[[GFS2 alloc]initWith:_add name:@"+"] atIndex:functions++ ];
                bin|=bit1;
            }
    
            if (([fs[@"sub"] isEqualTo:@YES])&&((![[Configuration all][@"forcebit2"]isEqualTo:@NO])||([[Configuration all][@"forcebit2"]isEqualTo:[NSNull null]]))) {
                [elements insertObject:[[GFS2 alloc]initWith:_sub name:@"-"] atIndex:functions++ ];
                bin|=bit2;
            }
            
            if (([fs[@"mul"] isEqualTo:@YES])&&((![[Configuration all][@"forcebit3"]isEqualTo:@NO])||([[Configuration all][@"forcebit3"]isEqualTo:[NSNull null]]))) {
                [elements insertObject:[[GFS2 alloc]initWith:_mul name:@"*"] atIndex:functions++ ];
                bin|=bit3;
            }
            
            
            if (([fs[@"div"] isEqualTo:@YES])&&((![[Configuration all][@"forcebit4"]isEqualTo:@NO])||([[Configuration all][@"forcebit4"]isEqualTo:[NSNull null]]))) {
        ...
        if ([[Configuration all][@"exponentialFunction"]isEqualTo:@YES]) {
            if (([fs[@"^"] isEqualTo:@YES])&&((![[Configuration all][@"forcebit125"]isEqualTo:@NO])||([[Configuration all][@"forcebit125"]isEqualTo:[NSNull null]]))) {
        ...
        if ([[Configuration all][@"goniometricFunctions"]isEqualTo:@YES]) {
            if (([fs[@"sin"] isEqualTo:@YES])&&((![[Configuration all][@"forcebit5"]isEqualTo:@NO])||([[Configuration all][@"forcebit5"]isEqualTo:[NSNull null]]))) {
                [elements insertObject:[[GFS1 alloc]initWith:_sin name:@"sin"] atIndex:functions++ ];
                bin|=bit5;
            }
            
            if (([fs[@"cos"] isEqualTo:@YES])&&((![[Configuration all][@"forcebit6"]isEqualTo:@NO])||([[Configuration all][@"forcebit6"]isEqualTo:[NSNull null]]))) {
                [elements insertObject:[[GFS1 alloc]initWith:_cos name:@"cos"] atIndex:functions++ ];
                bin|=bit6;
            }
            
            if (([fs[@"tan"] isEqualTo:@YES])&&((![[Configuration all][@"forcebit7"]isEqualTo:@NO])||([[Configuration all][@"forcebit7"]isEqualTo:[NSNull null]]))) {
                [elements insertObject:[[GFS1 alloc]initWith:_tan name:@"tan"] atIndex:functions++ ];
                bin|=bit7;
            }
            
        }
        if ([[Configuration all][@"logarithmicFunctions"]isEqualTo:@YES]) {
            if (([fs[@"log"] isEqualTo:@YES])&&((![[Configuration all][@"forcebit8"]isEqualTo:@NO])||([[Configuration all][@"forcebit8"]isEqualTo:[NSNull null]]))) {
                [elements insertObject:[[GFS1 alloc]initWith:_log name:@"log"] atIndex:functions++ ];
                bin|=bit8;
            }
            
            if (([fs[@"log2"] isEqualTo:@YES])&&((![[Configuration all][@"forcebit9"]isEqualTo:@NO])||([[Configuration all][@"forcebit9"]isEqualTo:[NSNull null]]))) {
                [elements insertObject:[[GFS1 alloc]initWith:_log2 name:@"log2"] atIndex:functions++ ];
                bin|=bit9;
            }
            
            if (([fs[@"log10"] isEqualTo:@YES])&&((![[Configuration all][@"forcebit10"]isEqualTo:@NO])||([[Configuration all][@"forcebit10"]isEqualTo:[NSNull null]]))) {
                [elements insertObject:[[GFS1 alloc]initWith:_log10 name:@"log10"] atIndex:functions++ ];
                bin|=bit10;
            }
        }
        if ([[Configuration all][@"polynomialFunctions"]isEqualTo:@YES]) {
            if (([fs[@"pow2"] isEqualTo:@YES])&&((![[Configuration all][@"forcebit11"]isEqualTo:@NO])||([[Configuration all][@"forcebit11"]isEqualTo:[NSNull null]]))) {
                [elements insertObject:[[GFS1 alloc]initWith:_pow2 name:@"^2"] atIndex:functions++ ];
                bin|=bit11;
            }
            
            if (([fs[@"pow3"] isEqualTo:@YES])&&((![[Configuration all][@"forcebit12"]isEqualTo:@NO])||([[Configuration all][@"forcebit12"]isEqualTo:[NSNull null]]))) {
                [elements insertObject:[[GFS1 alloc]initWith:_pow3 name:@"^3"] atIndex:functions++ ];
                bin|=bit12;
            }
            
            if (([fs[@"pow4"] isEqualTo:@YES])&&((![[Configuration all][@"forcebit13"]isEqualTo:@NO])||([[Configuration all][@"forcebit13"]isEqualTo:[NSNull null]]))) {
                [elements insertObject:[[GFS1 alloc]initWith:_pow4 name:@"^4"] atIndex:functions++ ];
                bin|=bit13;
            }
            
            if (([fs[@"pow5"] isEqualTo:@YES])&&((![[Configuration all][@"forcebit14"]isEqualTo:@NO])||([[Configuration all][@"forcebit14"]isEqualTo:[NSNull null]]))) {
                [elements insertObject:[[GFS1 alloc]initWith:_pow5 name:@"^5"] atIndex:functions++ ];
                bin|=bit14;
            }
            
            if (([fs[@"pow6"] isEqualTo:@YES])&&((![[Configuration all][@"forcebit15"]isEqualTo:@NO])||([[Configuration all][@"forcebit15"]isEqualTo:[NSNull null]]))) {
                [elements insertObject:[[GFS1 alloc]initWith:_pow6 name:@"^6"] atIndex:functions++ ];
                bin|=bit15;
            }
        }
        
        if ([[Configuration all][@"otherFunctions"]isEqualTo:@YES]) {
            
            if (([fs[@"sqrt"] isEqualTo:@YES])&&((![[Configuration all][@"forcebit26"]isEqualTo:@NO])||([[Configuration all][@"forcebit26"]isEqualTo:[NSNull null]]))) {
                [elements insertObject:[[GFS1 alloc]initWith:_sqrt name:@"sqrt"] atIndex:functions++ ];
                bin|=bit26;
            }
            
            if (([fs[@"abs"] isEqualTo:@YES])&&((![[Configuration all][@"forcebit27"]isEqualTo:@NO])||([[Configuration all][@"forcebit27"]isEqualTo:[NSNull null]]))) {
                [elements insertObject:[[GFS1 alloc]initWith:_abs name:@"abs"] atIndex:functions++ ];
                bin|=bit27;
            }
            
            if (([fs[@"e^"] isEqualTo:@YES])&&((![[Configuration all][@"forcebit28"]isEqualTo:@NO])||([[Configuration all][@"forcebit28"]isEqualTo:[NSNull null]]))) {
        ...
        if ([[Configuration all][@"linearConstants"]isEqualTo:@YES]) {
        
            if (([fs[@"1"] isEqualTo:@YES])&&((![[Configuration all][@"forcebit16"]isEqualTo:@NO])||([[Configuration all][@"forcebit16"]isEqualTo:[NSNull null]]))) {
                [elements insertObject:[[GFS0 alloc]initWith:_1 name:@"1"] atIndex:xpos++];
                bin|=bit16;
            }
            
            if (([fs[@"2"] isEqualTo:@YES])&&((![[Configuration all][@"forcebit17"]isEqualTo:@NO])||([[Configuration all][@"forcebit17"]isEqualTo:[NSNull null]]))) {
                [elements insertObject:[[GFS0 alloc]initWith:_2 name:@"2"] atIndex:xpos++];
                bin|=bit17;
            }
            
            if (([fs[@"3"] isEqualTo:@YES])&&((![[Configuration all][@"forcebit18"]isEqualTo:@NO])||([[Configuration all][@"forcebit18"]isEqualTo:[NSNull null]]))) {
                [elements insertObject:[[GFS0 alloc]initWith:_3 name:@"3"] atIndex:xpos++];
                bin|=bit18;
            }
            
            if (([fs[@"4"] isEqualTo:@YES])&&((![[Configuration all][@"forcebit19"]isEqualTo:@NO])||([[Configuration all][@"forcebit19"]isEqualTo:[NSNull null]]))) {
                [elements insertObject:[[GFS0 alloc]initWith:_4 name:@"4"] atIndex:xpos++];
                bin|=bit19;
            }
            
            if (([fs[@"5"] isEqualTo:@YES])&&((![[Configuration all][@"forcebit20"]isEqualTo:@NO])||([[Configuration all][@"forcebit20"]isEqualTo:[NSNull null]]))) {
                [elements insertObject:[[GFS0 alloc]initWith:_5 name:@"5"] atIndex:xpos++];
                bin|=bit20;
            }
            
            if (([fs[@"6"] isEqualTo:@YES])&&((![[Configuration all][@"forcebit21"]isEqualTo:@NO])||([[Configuration all][@"forcebit21"]isEqualTo:[NSNull null]]))) {
                [elements insertObject:[[GFS0 alloc]initWith:_6 name:@"6"] atIndex:xpos++];
                bin|=bit21;
            }
            
            if (([fs[@"7"] isEqualTo:@YES])&&((![[Configuration all][@"forcebit22"]isEqualTo:@NO])||([[Configuration all][@"forcebit22"]isEqualTo:[NSNull null]]))) {
                [elements insertObject:[[GFS0 alloc]initWith:_7 name:@"7"] atIndex:xpos++];
                bin|=bit22;
            }
            
            if (([fs[@"8"] isEqualTo:@YES])&&((![[Configuration all][@"forcebit23"]isEqualTo:@NO])||([[Configuration all][@"forcebit23"]isEqualTo:[NSNull null]]))) {
                [elements insertObject:[[GFS0 alloc]initWith:_8 name:@"8"] atIndex:xpos++];
                bin|=bit23;
            }
            
            if (([fs[@"9"] isEqualTo:@YES])&&((![[Configuration all][@"forcebit24"]isEqualTo:@NO])||([[Configuration all][@"forcebit24"]isEqualTo:[NSNull null]]))) {
                [elements insertObject:[[GFS0 alloc]initWith:_9 name:@"9"] atIndex:xpos++];
                bin|=bit24;
            }
        }
        
        if ([[Configuration all][@"mathConstants"]isEqualTo:@YES]) {
            if (([fs[@"pi"] isEqualTo:@YES])&&((![[Configuration all][@"forcebit29"]isEqualTo:@NO])||([[Configuration all][@"forcebit29"]isEqualTo:[NSNull null]]))) {
                [elements insertObject:[[GFS0 alloc]initWith:_pi name:@"pi"] atIndex:xpos++];
                bin|=bit29;
            }
            
            if (([fs[@"e"] isEqualTo:@YES])&&((![[Configuration all][@"forcebit30"]isEqualTo:@NO])||([[Configuration all][@"forcebit30"]isEqualTo:[NSNull null]]))) {
                [elements insertObject:[[GFS0 alloc]initWith:_e name:@"e"] atIndex:xpos++];
                bin|=bit30;
            }
            
            if (([fs[@"phi"] isEqualTo:@YES])&&((![[Configuration all][@"forcebit31"]isEqualTo:@NO])||([[Configuration all][@"forcebit31"]isEqualTo:[NSNull null]]))) {
                [elements insertObject:[[GFS0 alloc]initWith:_phi name:@"phi"] atIndex:xpos++];
                bin|=bit31;
            }
        }
        if ([[Configuration all][@"randomConstants"]isEqualTo:@YES]) {
        
            uint64 actualbit;
            int count=0;
            for (int i =33; i<65; i++) {
                actualbit=1 << i;
                if ((fsb&actualbit)==actualbit) {
                    bin|=actualbit;
                    _c[count]=[mersennetwister randomDoubleFrom:MINCONSTANT to:MAXCONSTANT];
                    [elements insertObject:[[GFSconst alloc]initWith:&_c[count++]] atIndex:xpos++];
                }
    ...
    */
    ...
    if ((*depth > MAXDEPTH)||(i > MAXWIDTH)) {
        ...
        *lastrepairing+=1;
        ...
        f=[elements objectAtIndex:array[i]];
    */
    if ((*last > MAXSIZE)||(*lastrepairing > MAXSIZE)) {
        
        f=elements[terminalsStartingIndex];
        array[i]=terminalsStartingIndex;
    }
    else if (*depth > MAXDEPTH) {
        *depth = 0;
        f=elements[repairing[(*lastrepairing)]];
        array[i]=repairing[*lastrepairing];
        *lastrepairing+=1;
    }
...
-(NSString *)describeA:(int[])_a withB:(int[])_b{
    ...
    [temp appendString:[NSString stringWithFormat:@"\n%llu\n[%@]\n[%@]\n%@\n%@\n%@",self.bin,buffer1,buffer2,[self stringA:&_a[0] withB:&_b[0]],[self teXStringA:&_a[0] withB:&_b[0]],@(fitness)]];
...
-(double)errorA:(int[])_a withB:(int[])_b{
    ...
    for (j=0; j<50; j++) {
        ...
        if ([[Configuration all][@"function2D"] isEqualTo:@"x^6-2*x^4+x^2"])
            db=(pow(dx,6)-(2*pow(dx,4))+pow(dx,2));
        else if ([[Configuration all][@"function2D"] isEqualTo:@"x^5−2*x^3+x"])
        ...
        */
        if ([[Configuration all][@"function2D"] isEqualTo:@"x"])
            db=dx;
...
- (instancetype)initWithConfiguration:(NSDictionary*) conf andGFS:(GFS*)gs{
    self = [super init];
    if (self) {
...
-(BOOL)insertFitness:(double)f string:(NSString*)s ofMethod:(NSString*)m{
    ...
    if ((f<best)&&(f!=NAN)) {
        ...
        [calculations setValue:s forKey:[NSString stringWithFormat:@"%@",@(f)]];
        best=f;
//        [lock lock];
...
-(NSString*)bestDescription{
    if (best!=FLT_MAX)
        return [NSString stringWithFormat:@"%@",calculations[[NSString stringWithFormat:@"%@", @(best)]]];
    else
        return @"";
...
-(NSString*)description{
    ...
    for (NSString *n in calculations.allKeys) {
        ...
        if (![[NSString stringWithFormat:@"%@",calculations[n]]isEqualToString:@"x"])
        
        [mutable appendString:[NSString stringWithFormat:@"%@",calculations[n]]];
    }
    return [NSString stringWithFormat:@"%@\n",mutable];
}
@end
